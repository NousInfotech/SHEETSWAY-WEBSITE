...
export default function IndustryAwareAI () {
  ...
  useEffect(() => {
    ...
    // Convert to milliseconds - use the exact same duration for both timer and animation
    const timeoutMs = Math.round(durationSec * 1000)
  ...
  // Helper function to get consistent duration
  const getDuration = index => {
    const configuredDuration = industryAwareData[index]?.duration
    const primarySrc =
      industryAwareData[index]?.images?.primarySrc?.toLowerCase?.() || ''
    const isVideo = primarySrc.endsWith('.webm')

    return Number.isFinite(configuredDuration) && configuredDuration > 0
      ? configuredDuration
      : isVideo && mediaDurations[index]
        ...
        <div className='max-w-6xl mx-auto flex flex-col lg:flex-row justify-between gap-8 sm:gap-10 md:gap-16 lg:gap-20'>
          ...
          >
            <div className='absolute inset-0 flex items-center justify-center'>
              ...
              <div
                className='relative p-4 sm:p-2 md:p-0'
                style={{
