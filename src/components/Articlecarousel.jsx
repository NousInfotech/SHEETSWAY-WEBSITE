    ...
    <>
      ...
      <div className="flex items-center justify-center w-full mt-4 space-x-2">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 rounded-full transition-all duration-300 ${currentIndex === index ? "bg-[#E8840C] w-4" : "bg-gray-300"
              }`}
            onClick={() => setCurrentIndex(index)}
          ></button>
        ))}
            ...
            <div key={index} className="min-w-full flex">
              <div className=" flex flex-col sm:flex-row bg-white">
                <div className="sm:w-1/2 p-8 text-left flex flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between mb-9">
                      ...
                      />
                      {/* Moved Go to Article button below the content */}
                    </div>
                    <div
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-2">
                        {article.gotoArticle && (
                          <a target="_blank" rel="noreferrer" href={article.href}>
                            <Button type="primary" className="!bg-[#E8840C] hover:!bg-[#E8840C]/85">Go to Article -&gt;</Button>
                          </a>
                        )}
                        {article.btnText && (
                ...
                <div className="sm:w-1/2">
                  {article.ytSrc ? (
                    <ResponsiveYouTube videoURL={article.ytSrc} imgSrc={article.image} className={"h-[500px] rounded-tr-2xl rounded-br-2xl overflow-hidden w-full"} />
                  ) : (
                    <img
                      src={article.image}
                      alt="Article"
