...
const ResponsiveYouTube = ({ videoURL, imgSrc, className = "", playBtn = false }) => {
    ...
    return (
        <div style={{ width: "100%", textAlign: "center", height: "100%" }}>
            <div className={`w-full h-full select-none relative overflow-hidden flex justify-center items-center ${className}`}>
                <img
                    src={imgSrc}
                    alt="Video Thumbnail"
                    className={`w-full h-full object-cover object-center block ${playBtn ? "brightness-75" : ""}`}
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <img
                        src="/play-buttton_14782545.png"
                        alt="Play video"
                        className="w-16 h-12 md:w-20 md:h-16 rounded-xl shadow-md"
                        style={{ cursor: "pointer" }}
                        onClick={() => setIsPopupOpen(true)}
                    />
                </div>
