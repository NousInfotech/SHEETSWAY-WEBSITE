// Google Apps Script for Email Collection - Fixed Version
...
function doGet(e) {
  try {
    ...
    if (!sheet) {
      return ContentService
        .createTextOutput('Error: Email Collection sheet not found. Please create a sheet named "Email Collection"')
        .setMimeType(ContentService.MimeType.TEXT);
    }
...
function setupSheet() {
  try {
    ...
    if (!sheet) {
      console.log('Creating Email Collection sheet...');
      ...
      console.log('Email Collection sheet already exists');
    }
  } catch (error) {
...
function testSpreadsheetAccess() {
  try {
    ...
    if (sheet) {
      ...
      return 'Success: Can access spreadsheet and sheet';
    } else {
      return 'Error: Email Collection sheet not found';
    }
  } catch (error) {
